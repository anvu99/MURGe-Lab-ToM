...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "eval_deepseek_gemma_nomem")
    if not job_id:
...
def main():
    ...
    csv_file = f"evaluation/metrics_log_hetero_deepseek7B_gemma9B_{job_id}_no_memories.csv"
    memory_log_file = f"evaluation/tom_memories_log_hetero_deepseek7B_gemma9B_{job_id}_no_memories.json"

    # -----------------------------------------------------------------------
    # Data
    # -----------------------------------------------------------------------
    logger.info("Loading MMLU Pro dataset...")
    questions = load_mmlu_pro(split="test", num_samples=500)   # TODO: increase to 100 for full run
    ...
    summary_file = f"evaluation/final_summary_hetero_deepseek7B_gemma9B_{job_id}_no_memories.json"
