...
Runs a 2-agent debate between two different model families:
  Agent_A (Agent_Qwen):  Qwen/Qwen2.5-7B-Instruct       — GPU 0  (~14 GB bf16)
...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "eval_qwen_llama_nomem")
    if not job_id:
...
def main():
    ...
    csv_file = f"evaluation/metrics_log_hetero_qwen7B_llama8B{job_id}_no_memories.csv"
    memory_log_file = f"evaluation/tom_memories_log_hetero_qwen7B_llama8B{job_id}_no_memories.json"
    ...
    #   GPU 0: Qwen-7B bf16   ~14 GB weights + ~3 GB KV cache = ~17 GB
    ...
    summary_file = f"evaluation/final_summary_hetero_qwen7B_llama8B{job_id}_no_memories.json"
