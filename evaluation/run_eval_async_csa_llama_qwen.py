...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "async_csa_llama_qwen")
    if not job_id:
        return
...
def main():
    ...
    base_name = f"async_csa_llama8B_qwen7B_{job_id}"
    ...
    logger.info("Loaded Llama on GPU 0.")
    ...
    logger.info("Loaded Qwen on GPU 1.")
    ...
    arena.num_rounds = 5
    arena.n_agents = 2
    arena.agents = [agent_llama, agent_qwen]
    arena.observer = None
    arena._llm_cache = {llama_model: llama_llm, qwen_model: qwen_llm}
