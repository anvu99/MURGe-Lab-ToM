#!/usr/bin/env python3
"""
Heterogeneous multi-agent debate evaluation (No Memories).

Runs a 2-agent debate between two different model families without theory of mind:
  Agent_A (Agent_Gemma):  google/gemma-2-9b-it            — GPU 0  (~18 GB bf16)
...
SLURM submission (2 GPUs, 128 GB RAM):
    sbatch evaluation/submit_eval_gemma_llama_no_memories.sl

Outputs:
  evaluation/metrics_log_hetero_gemma9B_llama8B_..._no_memories.csv
  evaluation/tom_memories_log_hetero_gemma9B_llama8B_..._no_memories.json
"""
...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "eval_gemma_llama_nomem")
    if not job_id:
...
def main():
    ...
    csv_file = f"evaluation/metrics_log_hetero_gemma9B_llama8B_{job_id}_no_memories.csv"
    memory_log_file = f"evaluation/tom_memories_log_hetero_gemma9B_llama8B_{job_id}_no_memories.json"

    # -----------------------------------------------------------------------
    # Data
    # -----------------------------------------------------------------------
    logger.info("Loading MMLU Pro dataset...")
    questions = load_mmlu_pro(split="test", num_samples=400)   # TODO: increase to 100 for full run
    ...
    logger.info("Initializing heterogeneous DebateArena (No Memory):")
    logger.info("  Agent_Gemma: %s  ->  GPU 0", gemma_model)
    ...
    arena = DebateArena(
        ...
        model_names=[gemma_model, llama_model],
        num_rounds=5,
        agent_names=["Agent_Gemma", "Agent_Llama"],
    ...
    for idx, question_data in enumerate(questions, start=1):
        ...
        # 4. Snapshot ToM memory. Even without memory models, keeping the empty dict structure
        # to prevent logging errors and maintain symmetric outputs per the setup.
        if idx % 10 == 0:
    ...
    summary_file = f"evaluation/final_summary_hetero_gemma9B_llama8B_{job_id}_no_memories.json"
