#!/usr/bin/env python3
"""
3-Agent specialized heterogeneous debate evaluation (NO MEMORIES).
...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "eval_3agent_specialized_nomem")
    if not job_id:
...
def main():
    ...
    base_name = f"3agent_specialized_{job_id}_no_memories"
    csv_file = f"evaluation/metrics_log_{base_name}.csv"
    memory_log_file = f"evaluation/tom_memories_log_3agent_specialized_{job_id}_no_memories.json"
