...
ORDER-FLIPPED version: Llama is Agent 0 (GPU 0), Qwen is Agent 1 (GPU 1).
Used to test whether agent ordering affects reasoning/debate outcomes,
compared against run_eval_async_tts_qwen_llama_no_memories.py.

Models:
  Agent_Llama: meta-llama/Llama-3.1-8B-Instruct  — GPU 0  (~16 GB bf16)
  Agent_Qwen:  Qwen/Qwen2.5-7B-Instruct           — GPU 1  (~14 GB bf16)
...
from architectures.async_debate import AsyncDebateArena
...
def truncate_slurm_err_log():
    ...
    job_name = os.environ.get("SLURM_JOB_NAME", "async_tts_llama_qwen_nomem")
    if not job_id:
        return
...
def main():
    ...
    base_name = f"async_tts_llama8B_qwen7B_{job_id}_no_memories"
    ...
    # -----------------------------------------------------------------------
    # Models  — ORDER FLIPPED vs. qwen_llama variant
    # -----------------------------------------------------------------------
    llama_model = "meta-llama/Llama-3.1-8B-Instruct"
    qwen_model  = "Qwen/Qwen2.5-7B-Instruct"

    agent_names = ["Agent_Llama", "Agent_Qwen"]
    agent_ids   = ["agent_0", "agent_1"]

    logger.info("Initializing TTS DebateArena (order-flipped):")
    logger.info("  Agent_Llama: %s  ->  GPU 0", llama_model)
    logger.info("  Agent_Qwen:  %s  ->  GPU 1", qwen_model)
